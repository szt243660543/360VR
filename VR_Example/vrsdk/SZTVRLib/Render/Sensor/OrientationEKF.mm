...
{
    return [self glMatrixFromSo3:&_so3SensorFromWorld];
}
...
- (void)processGyro:(GLKVector3)gyro time:(double)sensorTimeStamp
{
    if (_sensorTimeStampGyro != 0.0) {
        ...
        [self updateCovariancesAfterMotion];
        Matrix3x3d temp;
        temp.set(&_mQ);
        temp.scale(dT * dT);
        _mP.plusEquals(&temp);
...
- (void)processAcceleration:(GLKVector3)acc time:(double)sensorTimeStamp
{
    _vZ.set(acc.x, acc.y, acc.z);
    [self updateAccelerationCovariance:_vZ.length()];
    if (_alignedToGravity)
    {
        [self accelerationObservationFunctionForNumericalJacobian:&_so3SensorFromWorld result:&_vNu];
        ...
        for (int dof = 0; dof < 3; dof++)
        {
            ...
            Vector3d tempV;
            [self accelerationObservationFunctionForNumericalJacobian:&tempM result:&tempV];
...
- (void)accelerationObservationFunctionForNumericalJacobian:(Matrix3x3d *)so3SensorFromWorldPred result:(Vector3d *)result
{
