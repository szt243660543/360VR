...
#import "NSString+MJ2Extension.h"
...
@implementation NSObject (MJ2KeyValue)

#pragma mark - 错误
static const char MJ2ErrorKey = '\0';
+ (NSError *)mj_error
{
    return objc_getAssociatedObject(self, &MJ2ErrorKey);
}

+ (void)setMj_error:(NSError *)error
{
    objc_setAssociatedObject(self, &MJ2ErrorKey, error, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

#pragma mark - 模型 -> 字典时的参考
/** 模型转字典时，字典的key是否参考replacedKeyFromPropertyName等方法（父类设置了，子类也会继承下来） */
static const char MJ2ReferenceReplacedKeyWhenCreatingKeyValuesKey = '\0';

+ (void)mj_referenceReplacedKeyWhenCreatingKeyValues:(BOOL)reference
{
    objc_setAssociatedObject(self, &MJ2ReferenceReplacedKeyWhenCreatingKeyValuesKey, @(reference), OBJC_ASSOCIATION_ASSIGN);
}

+ (BOOL)mj_isReferenceReplacedKeyWhenCreatingKeyValues
{
    __block id value = objc_getAssociatedObject(self, &MJ2ReferenceReplacedKeyWhenCreatingKeyValuesKey);
    if (!value) {
        [self mj_enumerateAllClasses:^(__unsafe_unretained Class c, BOOL *stop) {
            value = objc_getAssociatedObject(c, &MJ2ReferenceReplacedKeyWhenCreatingKeyValuesKey);
...
- (instancetype)mj_setKeyValues:(id)keyValues context:(NSManagedObjectContext *)context
{
    ...
    MJ2ExtensionAssertError([keyValues isKindOfClass:[NSDictionary class]], self, [self class], @"keyValues参数不是一个字典");
    ...
    [clazz mj_enumerateProperties:^(MJ2Property *property, BOOL *stop) {
        @try {
            // 0.检测是否被忽略
            ...
            for (NSArray *propertyKeys in propertyKeyses) {
                value = keyValues;
                for (MJ2PropertyKey *propertyKey in propertyKeys) {
            ...
            // 2.复杂处理
            MJ2PropertyType *type = property.type;
            ...
            MJ2ExtensionBuildError([self class], exception.reason);
            MJ2ExtensionLog(@"%@", exception);
        }
    }];
...
+ (instancetype)mj_objectWithKeyValues:(id)keyValues context:(NSManagedObjectContext *)context
{
    // 获得JSON对象
    keyValues = [keyValues mj_JSONObject];
    MJ2ExtensionAssertError([keyValues isKindOfClass:[NSDictionary class]], nil, [self class], @"keyValues参数不是一个字典");
...
{
    MJ2ExtensionAssertError(filename != nil, nil, [self class], @"filename参数为nil");
...
{
    MJ2ExtensionAssertError(file != nil, nil, [self class], @"file参数为nil");
...
+ (NSMutableArray *)mj_objectArrayWithKeyValuesArray:(id)keyValuesArray context:(NSManagedObjectContext *)context
{
    ...
    // 1.判断真实性
    MJ2ExtensionAssertError([keyValuesArray isKindOfClass:[NSArray class]], nil, [self class], @"keyValuesArray参数不是一个数组");
    
    // 如果数组里面放的是NSString、NSNumber等数据
    if ([MJ2Foundation isClassFromFoundation:self]) return [NSMutableArray arrayWithArray:keyValuesArray];
...
{
    // 如果自己不是模型类, 那就返回自己
    MJ2ExtensionAssertError(![MJ2Foundation isClassFromFoundation:[self class]], (NSMutableDictionary *)self, [self class], @"不是自定义的模型类")
    ...
    [clazz mj_enumerateProperties:^(MJ2Property *property, BOOL *stop) {
        @try {
            ...
            if ([clazz mj_isReferenceReplacedKeyWhenCreatingKeyValues]) {
                ...
                [propertyKeys enumerateObjectsUsingBlock:^(MJ2PropertyKey *propertyKey, NSUInteger idx, BOOL *stop) {
                    // 下一个属性
                    MJ2PropertyKey *nextPropertyKey = nil;
                    if (idx != keyCount - 1) {
                    ...
                    if (nextPropertyKey) { // 不是最后一个key
                        ...
                        if (tempInnerContainer == nil || [tempInnerContainer isKindOfClass:[NSNull class]]) {
                            if (nextPropertyKey.type == MJ2PropertyKeyTypeDictionary) {
                                tempInnerContainer = [NSMutableDictionary dictionary];
                            } else {
                                tempInnerContainer = [NSMutableArray array];
                            }
                            if (propertyKey.type == MJ2PropertyKeyTypeDictionary) {
...
{
    // 0.判断真实性
    MJ2ExtensionAssertError([objectArray isKindOfClass:[NSArray class]], nil, [self class], @"objectArray参数不是一个数组");
