...
@interface MJ2Property()
@property (strong, nonatomic) NSMutableDictionary *propertyKeysDict;
@property (strong, nonatomic) NSMutableDictionary *objectClassInArrayDict;
@end

@implementation MJ2Property
...
{
    MJ2Property *propertyObj = objc_getAssociatedObject(self, property);
    if (propertyObj == nil) {
...
- (void)setProperty:(objc_property_t)property
{
    ...
    MJ2ExtensionAssertParamNotNil(property);
    ...
    _type = [MJ2PropertyType cachedTypeWithCode:code];
}
...
- (NSArray *)propertyKeysWithStringKey:(NSString *)stringKey
{
    ...
    for (NSString *oldKey in oldKeys) {
        ...
        if (start != NSNotFound) { // 有索引的key
            ...
            if (prefixKey.length) {
                MJ2PropertyKey *propertyKey = [[MJ2PropertyKey alloc] init];
            ...
            for (NSInteger i = 0; i<cmps.count - 1; i++) {
                MJ2PropertyKey *subPropertyKey = [[MJ2PropertyKey alloc] init];
                subPropertyKey.type = MJ2PropertyKeyTypeArray;
